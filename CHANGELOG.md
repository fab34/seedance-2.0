...
- Added `references/proposal-kit.md`: the proposal mode - a one-sentence **intent echo** in the user's own words before any proposal, 2-3 contrasting direction proposal cards (logline, story shape, how-it-is-filmed, feel) when an idea is open-ended, exactly one marked recommendation ("you decide" selects it), pre-decided production parameters behind each card so the winning card converts to a brief with zero further questions, and one-line archiving of unchosen proposals in story state.
- Added `references/delivery-package.md`: the final-deliverable standard - the generation-ready prompt plus shot/camera plan (moves with motivated endpoints), framing and composition notes, scene and light, script/beat plan with sound intent, and a story structure suggestion that proactively names the natural growth path (stronger ending, part two, multi-clip arc). Full package for production/sequence work, ~10-line compact package for fast-lane clips; a bare prompt string is never a valid deliverable. Includes a consistency rule (every part describes the same film as the prompt) and a register rule (plain words vs director language; the prompt itself always stays production language and inside budget).
- Wired both into the root Operating Loop (new Proposal step 11 and Delivery step 14), the Fast Lane (compact package with one alternate-direction sentence), the Load Map, `seedance-interview` (intent echo + proposal cards in Propose-Then-Adjust), `seedance-interview-short` (intent echo + one alternate direction even in fast mode), and the `seedance-prompt` output contract (production plan around the prompt).
